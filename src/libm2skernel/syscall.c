...
int handle_guest_syscalls() {
	...
	case syscall_code_inst_slice:
	{
		isa_ctx->instr_slice = isa_regs->ebx;
		break;
	}
	case syscall_code_read_write_virt:
    {
		void* buf;FILE* fp;
		uint32_t pbuf;
		fp = fopen("../../Sim_disk","ab+");
		char s = (char) isa_regs->ebx;
		int num_of_byte = (int) isa_regs->ecx;
		pbuf = isa_regs->edx;
		int di_num	= isa_regs->esi;
		int off_num = isa_regs->edi;
		fseek(fp,di_num*100+off_num,SEEK_SET);
		
		buf =  malloc(num_of_byte);
		
		if(s == 'w'){
			printf("here1\n");
			mem_read(isa_mem, pbuf,num_of_byte,buf);
			fwrite(buf,sizeof(char),num_of_byte,fp);
			printf("here2\n");
			ctx_set_status(isa_ctx,ctx_write);
			ctx_set_status(isa_ctx,ctx_suspended);
			struct my_interrupt I;
			I.pid = get_pid();
			I.n = INSTR_COUNTER + di_num;
			printf("here3\n");
			push_into_queue(&Q,I);
		}
		else if(s == 'r'){
			printf("here4\n");
			mem_write(isa_mem, pbuf,num_of_byte,buf);
			size_t res = fread(buf,sizeof(char),num_of_byte,fp);
			printf("here5\n");
			if(res != num_of_byte) perror("Not enough memory,110050066");
			ctx_set_status(isa_ctx,ctx_read);
			ctx_set_status(isa_ctx,ctx_suspended);
			struct my_interrupt I;
			I.pid = get_pid();
			I.n = INSTR_COUNTER + di_num;
